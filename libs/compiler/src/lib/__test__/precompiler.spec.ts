...
const log = new Logger({
  name: 'test_precompiler',
  minLevel: 3,
}) as Logger<ILogObj>;

const removePositions = (tree: Node) =>
  visit(tree, (node) => {
...
describe('precompiler', () => {
  it('should export a single page', async () => {
    ...
    expect(page.root).toEqual(
      u('root', [
        u('heading', { depth: 1 }, [u('text', 'Hello')]),
        u('paragraph', [u('text', 'how are you?')]),
      ]),
    );
  ...
  describe('implementation', () => {
    ...
    it('should find headings and frontmatter', async () => {
      const mdast = u('root', [
        u('yaml', { value: 'title: Hello' }),
        u('heading', { depth: 1 }, [u('text', 'Hello')]),
        u('paragraph', [u('text', 'how are you?')]),
      ]) as Root;
      const scannedNodes = collectNodesOfInterest(
        mdast,
        matchDirectives,
        matchDestination,
        log,
    ...
    it('should attach local metadata to the page', async () => {
      const mdast = u('root', [
        u('yaml', { value: 'title: Hello' }),
        u('heading', { depth: 1 }, [u('text', 'Hello')]),
        u(IN_PAGE_DIRECTIVE, { name: 'title' }, [
          u('text', { value: 'Greetings, program' }),
        ]),
        u('paragraph', [u('text', 'how are you?')]),
      ]) as Root;
      const scannedNodes = collectNodesOfInterest(
      ...
      expect(pages[0].heading).toEqual(
        u('heading', { depth: 1 }, [u('text', 'Hello')]),
      );
